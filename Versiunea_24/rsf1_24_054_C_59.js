(function ($) {
  var completedTables = {
    'processed': false, 'tables': {}
  };
  var idnoCuiioExceptionList = [];
  var _isEntityMediumLarge = false;
  var _isEntityMediumMicro = false;

    Drupal.behaviors.rsf1 = {
        attach: function (context, settings) {
            // The rsf1 object is optional. If it is not supplied by the platform,
            // the special 57-005 / 57-010 exception logic is skipped.
            idnoCuiioExceptionList = [];
            if (typeof rsf1 !== 'undefined' && rsf1) {
                Object.keys(rsf1).forEach(function (key) {
                    idnoCuiioExceptionList.push({
                        IDNO: Number(rsf1[key].idno),
                        CUIIO: Number(rsf1[key].cuiio),
                        start: rsf1[key].start_date,
                        end: rsf1[key].end_date,
                        periodYear: Number(rsf1[key].year)
                    });
                });
            }
          ...
          jQuery('#mywebform-edit-form').on('keypress', 'input.numeric, input.float, input.money', function (event) {
            ...
            if (!Drupal.settings.mywebform.preview) {
                var exceptionMatch = isTaxpayerInExceptionList(false);
                var allowedStartDate;
                var allowedEndDate;
                ...
                $("#dec_period_from", context)
                    .datepicker("option", "minDate", allowedStartDate)
                    .datepicker("option", "maxDate", allowedEndDate);

                $("#dec_period_to", context)
                    .datepicker("option", "minDate", allowedStartDate)
                    .datepicker("option", "maxDate", allowedEndDate);
            ...
            $("#dec_period_to", context).on("change", function () {
                ...
                $("#nalogPeriodYear", context).val(year).trigger("change");
            });
        }
    };
    ...
    function isTaxpayerInExceptionList(useCuiio) {
        if (typeof useCuiio === 'undefined') {
            useCuiio = true;
        }

        // No rsf1 data -> no special exception validation.
        if (!idnoCuiioExceptionList.length) {
            return false;
        }

        var idnoValue = jQuery('#dec_fiscCod_fiscal').val();
        var cuiioValue = jQuery('#dec_fiscCod_cuiio').val();
        var IDNO = Number(idnoValue ? idnoValue.trim() : '');
        var CUIIO = Number(cuiioValue ? cuiioValue.trim() : '');

        var match = idnoCuiioExceptionList.find(function (entry) {
            return entry.IDNO === IDNO && (!useCuiio || entry.CUIIO === CUIIO);
        });
    ...
    function validate_57_005_exception() {
        // Daca rsf1 lipseste (sau contribuabilul nu este in lista),
        // se aplica validarea generala 57-005.
        if (!idnoCuiioExceptionList.length) {
            return false;
        }
        ...
        if (startDate.toLocaleDateString() !== match.allowedStartDate.toLocaleDateString()) {
            webform.errors.push({
                ...
                'msg': concatMessage(
                    '57-005',
                    '',
                    Drupal.t('Data începutului perioadei de raportare nu este corectă')
                ),
            });
        }
    ...
    function validate_57_002(values) {
         ...
         *
         * Daca toate conditiile sunt indeplinite, 57-002 nu se afiseaza.
         */
        ...
        // Logica normala 57-002 ramane neschimbata.
    ...
    // ----------------------------------------------------
    function validate_57_010(values) {
        var currentDate = new Date();
         ...
         * 4. dec_period_to nu este mai mare decat data curenta.
         *
         * Daca toate conditiile sunt indeplinite, 57-010 nu se afiseaza.
         */
        var exceptionStartDate = new Date(2026, 7, 1, 0, 0, 0);
        var exceptionEndDate = new Date(2026, 11, 31, 23, 59, 59);
        ...
        /*
         * Exceptia existenta pentru contribuabilii din obiectul rsf1:
         * pentru ei dec_period_to trebuie sa coincida cu allowedEndDate.
         */
        if (idnoCuiioExceptionList.length) {
            var match = isTaxpayerInExceptionList();

            if (match && endPeriod.length === 3) {
                ...
                if (endDate.toLocaleDateString() !== match.allowedEndDate.toLocaleDateString()) {
                    webform.warnings.push({
                        ...
                        'msg': concatMessage(
                            '57-010',
                            '',
                            Drupal.t('Data sfârșitului perioadei de raportare nu este corectă')
                        ),
                    });
                }
        ...
        // Logica generala 57-010 ramane neschimbata.
        if (endPeriod.length === 3) {
            ...
            if (
                (values.dec_lichidare && periodToStr >= comparedDateStr) ||
                (!values.dec_lichidare && periodToStr !== comparedDateStr)
            ) {
                webform.warnings.push({
                    'fieldName': 'dec_period_to',
...
//-----------------------------------------------------------------------
        ...
        // 57-002
        validate_57_002(values);

        var nalogPeriodYear = Drupal.settings.mywebform.values.nalogPeriodYear;
        var fileCnt = 0;
        ...
        if (!fileCnt || (isEntityMediumLarge() && fileCnt < 3)) {
            ...
            // 57-004 runs only when rsf1_entities_medium_micro exists
            // and the current entity is present in that object.
            webform.errors.push({
                'fieldName': '',
                'index': 0,
                'weight': 3,
                'msg': concatMessage('57-004', '', Drupal.t('Lipsește Nota informativă sau Raportul auditorului')),
            });
        }

// -------------------------------------------------------------------------------------

        if (!validate_57_005_exception()) {
            var periodToStr = endPeriod[2] + '-' + endPeriod[1] + '-' + endPeriod[0];
...
//------------------------------------------------------------------------
    ...
    function detectMediumMicroEntity() {
        ...
        // The object is optional. If it is absent, 57-004 is simply skipped.
        if (typeof rsf1_entities_medium_micro === 'undefined' || !rsf1_entities_medium_micro) {
            return;
        }

        var entities = Object.values(rsf1_entities_medium_micro);
