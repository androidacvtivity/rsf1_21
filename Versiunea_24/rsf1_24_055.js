          ...
          jQuery('#mywebform-edit-form').on('keypress', 'input.numeric, input.float, input.money', function (event) {
            ...
            if (!Drupal.settings.mywebform.preview) {
                ...
                $("#dec_period_from", context)
                    .datepicker("option", "minDate", new Date(periodInfo.start.year, periodInfo.start.month - 1, periodInfo.start.day))
                    .datepicker("option", "maxDate", new Date(periodInfo.end.year, periodInfo.end.month - 1, periodInfo.end.day));
